...
public class Deck {
    ...
    public void shuffle() {
        ...
        for (int i = 0; i < setOfCards.nextInt(); i++) { // for-loop : iterating through the cards and shuffling them  -
            int shuffledCards = setOfCards.nextInt(cards.size()); // this line generates a random index within the range of the current number if cards in the deck (here: cards.size()). This index will be used to select a random card to swap with the card at the current index ('i') during shuffling //
